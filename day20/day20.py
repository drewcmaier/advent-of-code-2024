...
def points_within_range(start: Point, dist: int):
    ...
    for i in range(-dist, dist + 1):
        ...
        for j in range(-j_range, j_range + 1):
            points.add((start[0] + i, start[1] + j))
...
def run_cheats(
    ...
    def find_cheat_points() -> set[Point]:
        ...
        for p in points_within_range(root, max_len):
            if can_move_to(p, grid, dim):
                start_to_p = dist_to[p]
                p_to_end = path_len - start_to_p
                cheat_len = point_len + p_to_end + manhattan_dist(p, root)
...
def part1():
    ...
    return sum(run_cheats(grid, dim, p, path, dist_to, 2, 100) for p in path)


def part2():
    grid, dim, start, end = parse_input("input.txt")

    astart_start = perf_counter()
    path, dist_to = a_star(grid, dim, start, end)
    astart_end = perf_counter()

    cheat_start = perf_counter()
    total = sum(run_cheats(grid, dim, p, path, dist_to, 20, 100) for p in path)
    cheat_end = perf_counter()

    print(f"A*: {astart_end-astart_start}, cheats: {cheat_end-cheat_start}")
